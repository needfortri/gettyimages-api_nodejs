...
function WebHelper(credentials, hostName) {
    ...
    this.get = async function (path,headers) {
        ...
        if (credentials.apiKey && (credentials.apiSecret || credentials.forceAccessToken)) {
    ...
    function addUserAgentString(options) {
        // Following line is making pre-flight request fails on Firefox because of CORS issue
